...
GLuint BuildTrianglesFor0(float desloc_x);
GLuint BuildTrianglesFor1(float desloc_x);
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(500, 500, "INF01047 - 302229 - Gabriel Couto Domingues", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    GLuint vertex_array_object_id_0[4];
    GLuint vertex_array_object_id_1[4];
    float desloc_x = 0;
    for(int i = 0; i < 4; ++i){
        vertex_array_object_id_0[i] = BuildTrianglesFor0(desloc_x);
        vertex_array_object_id_1[i] = BuildTrianglesFor1(desloc_x);
        desloc_x -= 0.5;
    }
...
GLuint BuildTrianglesFor0(float desloc_x)
{
    ...
    int i = 0;
    double PI = acos(-1);
    for (double x = 0.0; x < 360.0; x += 22.5){
        double r = x * PI / 180.0;
        NDC_coefficients[4*i] = 0.75f + desloc_x + 0.2*cos(r);
        NDC_coefficients[4*i + 1] = 0.5*sin(r);
        ++i;
    }

    for (double x = 0.0; x < 360.0; x += 22.5){
        double r = x * PI / 180.0;
        NDC_coefficients[4*i] = 0.75f + desloc_x + 0.1*cos(r);
        NDC_coefficients[4*i + 1] = 0.4*sin(r);
        ++i;
    }
...
GLuint BuildTrianglesFor1(float desloc_x)
{
    ...
    GLfloat NDC_coefficients[] = {
    //    X      Y     Z     W
         0.65f + desloc_x,  -0.5f, 0.0f, 1.0f,
         0.65f + desloc_x,   0.35f, 0.0f, 1.0f,
         0.55f + desloc_x,   0.3f, 0.0f, 1.0f,
         0.65f + desloc_x,   0.5f, 0.0f, 1.0f,
         0.85f + desloc_x,   0.5f, 0.0f, 1.0f,
         0.85f + desloc_x,  -0.5f, 0.0f, 1.0f,
    };
    ...
    // Este vetor "indices" define a TOPOLOGIA (veja slides 64-71 do documento Aula_04_Modelagem_Geometrica_3D.pdf).
    //
    GLubyte indices[] = {1,2,3,  1,3,4,  0,1,4,  5,0,4}; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
